...
Status: v1 foundation, AI chat, multi-document UI, and multi-user persistence are live. The chat is catalog-aware and the preview pane switches per document — picking any of the 11 catalog docs renders its underlying PRC-law Chinese template with AI-collected key terms. All 11 catalog documents are manifest-driven and kernel-managed: `templates/manifests/<doc_id>.json` drives the LLM field checklist, server-owned field states, the edit form, structured Cover Page preview, term-reference highlighting, download gating, and authenticated server-side DOCX/PDF export. Both formats share one confirmed-value-only semantic render model; DOCX is the default editable download. The generic renderer remains only as a fallback for unknown or unmanaged document ids. Official source captures and adaptation limits for the two official-baseline documents are recorded in `templates/sources/` and ADR 0002; the remaining Prelegal v1.0 templates are their product-selected final baselines and do not depend on external source retrieval. Requests outside the catalog get routed to the closest available item. Real password auth (bcrypt + bearer-token sessions) gates per-user document CRUD and AI chat; drafts auto-save (debounced, 800ms) including the conversation log and non-field metadata, while manifest-managed fields are written through field patches. Drafts show up in a left sidebar and survive container restarts via a host-mounted SQLite volume. The frontend remembers the user's last open draft and restores it (with chat history replayed) on refresh / re-login. Upstream LLM errors are classified into one-line user-facing messages instead of dumping raw exception traces. A four-place disclaimer (preview banner, page footer, login marketing column, exported files) identifies the document as an AI draft, disclaims legal advice, and directs users to licensed-lawyer review before signing.
...
backend/         FastAPI service (uv project)
  ...
  app/export.py    Shared confirmed-value render model + DOCX (python-docx)
                   and PDF (WeasyPrint) file renderers
  ...
  quality_evals/   deterministic 11-document kernel/download/export corpus;
                   Python tripwires plus a kernel-isolated Linux Docker gate
...
- `GET /api/documents/{id}/download?format=docx|pdf` → authenticated server-rendered file using confirmed kernel values. Required fields are gated with 409 and `unresolved_required_fields`; cross-user access is 404. DOCX uses python-docx; PDF uses WeasyPrint with Noto CJK fonts.
